...
本章介绍Spring框架中控制反转（IOC）容器的实现原理，IOC也被成为依赖注入（DI）。在这个过程中，对象仅能通过构造函数参数、工厂方法参数以及属性来定义他们的依赖关系（也就是说，他们使用的是其他对象）并且只有在构造或者工厂方法有返回值后才能设置对象实例。当容器创建bean时注入这些依赖。此过程从根本上通过直接使用类构造或服务定位模式机制来控制实例化或定位它的的依赖，对于bean本身是一个逆向操作（故称之为控制反转）。
...
对于Spring IOC容器而言， `org.springframework.beans` 和 `org.springframework.context` 包是容器的基础。{api-spring-framework}/beans/factory/BeanFactory.html[`BeanFactory`] 接口提供了以中国能够管理任意类型对象的高级配置机制。
...
* 消息资源处理（用于国际化）
* 事件发布
* 应用层特定上下文，比如：在Web应用程序中使用 `WebApplicationContext`
...
简而言之， `BeanFactory` 提供了配置框架和基础功能，同时也在 `ApplicationContext` 中添加了更多的特定企业功能。 `ApplicationContext` 是 `BeanFactory`的一个完美超集，并且在本章节中仅用作描述Spring IOC容器。有关 `BeanFactory` 而不是 `ApplicationContext` 的更多使用信息，请参考<<beans-beanfactory>> 。
...
在Spring中，组成应用程序主干并且由Spring IOC容器管理的对象被称之为Bean。Bean是一个通过Spring IOC容器进行实例化、组装以及其他方式管理的对象，Bean与他们的依赖关系反映在容器使用的配置元数据中。
...
`org.springframework.context.ApplicationContext` 接口代表Spring IOC容器并且负责实例化、配置和组装Bean，容器通过读取配置元数据来获取相关指令对对象进行实例化、配置还是组装。配置元数据以XML、Java注解或者Java代码的形式展现，它是你能够展现出你所编写的程序对象以及这些对象之间丰富的相关依赖关系。
...
Spring提供了 `ApplicationContext` 接口的集中实现。在独立应用中，通常使用{api-spring-framework}/context/support/ClassPathXmlApplicationContext.html[`ClassPathXmlApplicationContext`]
或 {api-spring-framework}/context/support/FileSystemXmlApplicationContext.html[`FileSystemXmlApplicationContext`] 创建实例。XML是定义配置元数据的传统方式，但是你也可以指示容器使用Java注解或代码作为元数据，通过提供少量XML配置声明启用支持这些额外的元数据形式。
...
在多数的应用场景中，不需要显示的用户代码在Spring IoC容器中实例化一个或多个实例。在Web应用场景中，应用中的 `web.xml` 文件通常只要约简单8行XML web描述模板（请参考 <<context-create>>）。如果你使用 https://spring.io/tools/sts[Spring Tool Suite]（Eclipse驱动的开发环境），你可以通过少量的鼠标或键盘操作就可以轻易的生成配置模板了。
...
下图展示的是一张Spring的工作原理图。在 `ApplicationContext` 创建和初始化后，你的应用程序类和配置元数据就组合在一起了，此时你就拥有了一个完整配置且可执行的系统或应用程序。
...
如上图所示，Spring IoC容器使用一种形式的配置元数据。作为一个应用开发者，配置元数据代表着在你的应用程序中你告知Spring IoC容器该如何对对象实例化、配置以及组装。

Configuration metadata is traditionally supplied in a simple and intuitive XML format,
which is what most of this chapter uses to convey key concepts and features of the
Spring IoC container.

传统意义上，配置元数据以简单且直观的XML形式提供，这也是本章大部分内容描述Spring IoC的关键概念和功能。
...
NOTE: 基于XML的元数据不是元数据配置的唯一允许形式。Spring IoC容器本身实际上也是与写入的配置元数据完全解耦的。这些日子以来，许多开发人员为他们的Spring应用程序选择使用<<beans-java, Java-based configuration>>
...
有关Spring容器使用其他的元数据形式信息，请参考：

* <<beans-annotation-config,Annotation-based configuration>>: Spring 2.5引入了基于注解的配置元数据支持。
* <<beans-java, Java-based configuration>>: 从Spring 3.0开始，Spring JavaConfig项目提供的许多功能成为了Spring框架核心的一部分。从而你可以通过使用Java配置而不是XML文件来定义你的应用程序之外的Bean。想要使用这些功能，请参考注解：
  https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Configuration.html[`@Configuration`],
  https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Bean.html[`@Bean`],
  https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Import.html[`@Import`],
和 https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/DependsOn.html[`@DependsOn`]
...
Spring配置由一个（通常一个以上）Bean定义组成，并且必须容器管理。基于XML配置元数据配置这些bean作为顶级 `<beans/>` 元素内的 `<bean/>` 元素。Java配置通常在 `@Configuration` 中使用 `@Bean` 注解方法。
...
这些bean对应着构建应用的实际对象。通常体现在你定义的服务层对象、数据访问对象（DAO）、表现层对象（如：Structs `Action` 实例）、基础结构对象（如：Hibernate 的 `SessionFactories`，JMS `Queues`等）。因为创建和加载域对象的是DAO和业务逻辑的职责，通常不会在容器中配置细粒度的域对象。但是你可以在创建IoC容器控制之外的对象时通过Spring集成AspectJ来配置。请参考<<aop-atconfigurable,Using AspectJ to
dependency-inject domain objects with Spring>>：

The following example shows the basic structure of XML-based configuration metadata:

以下示例展示了基于XML配置元数据的基础结构：
...
<1> `id` 属性是字符型，用来标识单个bean定义。

<2> `class` 属性定义bean类型和使用的完全限定类名。
...
`id` 属性值关联着协作对象，在此例子中XML没有展示引用协作对象内容。请参考 <<beans-dependencies,Dependencies>> 获取更多信息：
...
提供给 `ApplicationContext` 构造函数一个或多个位置路径是资源字符串，可以让容器加载各种额外资源配置元数据，例如：本地文件系统、Java `CLASSPATH` 等。
...
[NOTE]
====
在学习了Spring IoC容器后，你可能想了解更多关于Spring的 `Resource` 抽象(如 <<resources>> 中所述)，并且提供了一个简单的机制从URI语法位置定义中读取输入流。尤其是使用 `Resource` 路径构造应用上下文，如 <<resources-app-ctx>> 中所述。
====

The following example shows the service layer objects `(services.xml)` configuration file:

以下示例展示的是服务层对象 `(services.xml)` 配置文件：
...
在前面的例子中，服务层是由 `PetStoreServiceImpl` 类以及 两个类型为 `JpaAccountDao` 和 `JpaItemDao` 对象组成（基于JPA的标准对象关系映射）。 `property name` 元素指的是JavaBean的属性名，`ref` 元素对应的是其他的Bean定义。  `id` 和 `ref` 元素之间的联系体现了协作对象之间的依赖关系。有关配置对象依赖关系的详细信息，请参考：<<beans-dependencies,Dependencies>>
...
让bean定义跨多个XML文件使用是很有用的，通常在应用架构中每个单独的配置文件代表着一个逻辑层或模块。
...
你可以使用应用上下文构造器从这些XML片段中加载Bean定义。如上一节<<beans-factory-instantiation>> 中所描述，构造函数有多个`Resource` 路径。也可以使用一个或多个 `<import/>` 标签从一个或多个文件中加载Bean定义。如下示例所示：
...
在前面的例子中，外部化bean定义 是从`services.xml`, `messageSource.xml`, and `themeSource.xml` 三个文件中加载的。所有位置路径都是相对于进行导入的定义文件，所以 `service.xml` 必须跟导入的文件在同一个目录或类路径下，而 `messageSource.xml` 和 `themeSource.xml` 必须导入文件路径在 `resources` 位置下。正如你所见，资路径的前斜杠是被忽略的。鉴于资源路径是相对的，最好不要使用斜杠。根据Spring Schema导入的文件内容（包括顶级标签 `<beans/>`）必须符合XML Bean定义。
...
[NOTE]
====
可以使用 "../" 相对路径引用父目录中的文件（不推荐使用这种方式），会创建一个当前程序意外的文件依赖。
====
